...
Dependencies:
    ...
    torch
    mrcfile
    tifffile          # TIFF metadata and CPU decode fallback
    imagecodecs       # CPU decode fallback for compressed TIFF variants
    nvidia-nvimgcodec-cu12[nvtiff]  # optional nvTIFF GPU decode (CUDA 12)

Supported input:
    2-D MRC image, 3-D MRC movie stack (Z, Y, X), or TIFF movie stack
    (.tif/.tiff). Multi-page TIFF stacks are read frame-by-frame. On CUDA,
...
class MovieSource:
    ...
    MRC files are memory-mapped through mrcfile. TIFF metadata and the CPU
    fallback are handled by tifffile/imagecodecs. For ordinary multi-page
    grayscale TIFF movies on a CUDA device, the optional nvImageCodec Python
    frontend is used with a GPU_ONLY backend, which dispatches TIFF decoding to
    nvTIFF. The decoded image is transferred to PyTorch through DLPack without a
    host copy. Any initialization/decode error, including CUDA OOM, disables the
    GPU TIFF decoder for the rest of the run and retries the frame on CPU.
    """
    ...
    def __init__(
        ...
        tiff_decoder: str = "auto",
    ) -> None:
        ext = Path(filename).suffix.lower()
        if ext in {".eer", ".dm4"}:
            raise ValueError(f"{ext} input is not supported. Convert the movie to MRC or TIFF first.")
        if tiff_decoder not in {"auto", "nvtiff", "cpu"}:
            raise ValueError("tiff_decoder must be one of: auto, nvtiff, cpu")
        ...
        self._tiff_decoder_requested = tiff_decoder
        self._tiff_backend = "cpu"
        self._nvimgcodec = None
        self._nv_decoder = None
        self._nv_codestream = None
        self._nv_decode_params = None
        self._nvtiff_frame_cache: dict[int, Tensor] = {}
        ...
        if self.kind == "mrc":
            ...
            # tifffile is retained even when nvTIFF is used: it provides robust
            # layout metadata and is the mandatory automatic CPU fallback.
            tifffile = _import_tifffile()
        ...
        if self.kind == "tiff":
            self._initialize_nvtiff_backend()

    @property
    def tiff_backend(self) -> str:
        return self._tiff_backend

    def _initialize_nvtiff_backend(self) -> None:
        if self._tiff_decoder_requested == "cpu":
            log("TIFF decoder: CPU tifffile/imagecodecs (--tiff-decoder cpu)")
            return
        if self.device.type != "cuda":
            log("TIFF decoder: CPU tifffile/imagecodecs (processing device is not CUDA)")
            return
        if self._tiff_mode not in {"pages", "single2d"}:
            log(
                "TIFF decoder: CPU tifffile/imagecodecs; nvTIFF is not enabled for "
                f"the shaped-series layout {self._tiff_mode!r}"
            )
            return
        ...
        try:
            ...
            log(
                "WARNING: nvTIFF GPU decoding is unavailable; using CPU "
                "tifffile/imagecodecs. Install the CUDA-matched package, e.g.\n"
                "         pip install 'nvidia-nvimgcodec-cu12[nvtiff]'\n"
        ...
        try:
            ...
            # Current nvImageCodec releases require Backend objects. The enum
            # fallback keeps compatibility with older beta Python bindings.
            try:
                backend = nvimgcodec.Backend(gpu_only)
                self._nv_decoder = nvimgcodec.Decoder(
            ...
            self._nv_codestream = nvimgcodec.CodeStream(self.filename)
            available_images = getattr(self._nv_codestream, "num_images", None)
            if available_images is not None and int(available_images) < self.n_frames:
                raise RuntimeError(
                    f"nvImageCodec sees only {int(available_images)} image(s), but "
                    f"tifffile reports {self.n_frames} movie frame(s)"
                )
            unchanged = getattr(
                nvimgcodec.ColorSpec,
                "UNCHANGED",
                nvimgcodec.ColorSpec.UNKNOWN,
            )
            self._nv_decode_params = nvimgcodec.DecodeParams(
                apply_exif_orientation=False,
                allow_any_depth=True,
                color_spec=unchanged,
            )
            self._nvimgcodec = nvimgcodec
            self._tiff_backend = "nvtiff"

            # Decode frame 0 once as an early capability/OOM probe. The tensor is
            # cached and consumed by the first call to frame(0), so no work is lost.
            self._nvtiff_frame_cache[0] = self._decode_nvtiff_frame(0)
            log("TIFF decoder: nvTIFF GPU via nvImageCodec (GPU_ONLY; CPU fallback enabled)")
        except Exception as exc:
            self._switch_tiff_to_cpu(exc, frame_index=0, during_initialization=True)

    def _switch_tiff_to_cpu(
        self,
        exc: BaseException,
        frame_index: int,
        during_initialization: bool = False,
    ) -> None:
        oom = _looks_like_cuda_oom(exc)
        phase = "initialization/probe" if during_initialization else f"frame {frame_index + 1}"
        if oom:
            log(
                "WARNING: nvTIFF GPU decoding ran out of CUDA memory during "
                f"{phase}. Releasing nvTIFF resources and automatically switching "
                "all remaining TIFF reads to CPU tifffile/imagecodecs."
            )
        else:
            log(
                "WARNING: nvTIFF GPU decoding failed during "
                f"{phase}; automatically switching all remaining TIFF reads to "
                "CPU tifffile/imagecodecs.\n"
                f"         {type(exc).__name__}: {exc}"
            )

        self._tiff_backend = "cpu"
        self._nvtiff_frame_cache.clear()
        self._nv_decode_params = None
        self._nv_codestream = None
        self._nv_decoder = None
        self._nvimgcodec = None
    ...
    def _tiff_frame_cpu(self, index: int) -> np.ndarray:
    ...
    def _decode_nvtiff_frame(self, index: int) -> Tensor:
        if self._nv_decoder is None or self._nv_codestream is None:
            raise RuntimeError("nvTIFF decoder is not initialized")
        try:
            substream = self._nv_codestream.get_sub_code_stream(image_idx=int(index))
        except TypeError:
            # Compatibility with nvImageCodec releases using a positional index.
            substream = self._nv_codestream.get_sub_code_stream(int(index))

        stream_ptr = int(torch.cuda.current_stream(self.device).cuda_stream)
        try:
            decoded = self._nv_decoder.decode(
                substream,
                params=self._nv_decode_params,
                cuda_stream=stream_ptr,
            )
        except TypeError:
            # Older bindings may not expose the cuda_stream keyword.
            decoded = self._nv_decoder.decode(substream, params=self._nv_decode_params)
        if decoded is None:
            raise RuntimeError("nvImageCodec returned None for this TIFF frame")

        if hasattr(decoded, "to_dlpack"):
            tensor = torch.utils.dlpack.from_dlpack(decoded.to_dlpack())
        else:
        ...
        if tensor.ndim != 2:
            raise ValueError(
                "nvTIFF decoded a non-grayscale frame with shape "
                f"{tuple(tensor.shape)}; cryo-EM input must contain one channel"
            )
        if tuple(tensor.shape) != (self.height, self.width):
            raise ValueError(
                f"nvTIFF frame {index} has shape {tuple(tensor.shape)}; "
                f"expected {(self.height, self.width)}"
            )
        if tensor.device.type != "cuda":
            raise RuntimeError(
                f"GPU_ONLY nvTIFF decode unexpectedly returned a tensor on {tensor.device}"
            )
        expected_device = self.device.index
        if expected_device is None:
            expected_device = torch.cuda.current_device()
        if tensor.device.index != int(expected_device):
            raise RuntimeError(
                f"nvTIFF returned {tensor.device}, but processing uses cuda:{expected_device}"
            )
        # Keep the decoder's native row stride here. frame() converts directly
        # into one contiguous float32 tensor, avoiding an extra uint16 copy.
        return tensor

    def _tiff_frame_gpu(self, index: int) -> Tensor:
        cached = self._nvtiff_frame_cache.pop(index, None)
        if cached is not None:
            return cached
        return self._decode_nvtiff_frame(index)

    def _cpu_array_to_device_tensor(self, raw: np.ndarray, index: int) -> Tensor:
        arr = np.array(raw, dtype=np.float32, copy=True, order="C")
        if arr.ndim != 2:
            raise ValueError(
                f"Frame {index} from {self.filename} is not 2-D after decoding; "
                f"shape {arr.shape}"
            )
    ...
    def frame(self, index: int, remove_mean: bool = True) -> Tensor:
        ...
        if self.kind == "mrc":
            ...
            image = self._cpu_array_to_device_tensor(raw, index)
        elif self._tiff_backend == "nvtiff":
            retry_on_cpu = False
            raw_gpu: Optional[Tensor] = None
            try:
                raw_gpu = self._tiff_frame_gpu(index)
                image = raw_gpu.to(
                    dtype=torch.float32,
                ...
                # Drop the DLPack tensor before destroying the decoder. The
                # actual CPU retry happens after leaving this except block so
                # the exception traceback cannot retain failed GPU buffers.
                raw_gpu = None
                self._switch_tiff_to_cpu(exc, frame_index=index)
                retry_on_cpu = True
            if retry_on_cpu:
                gc.collect()
                release_memory(self.device)
                try:
                    image = self._cpu_array_to_device_tensor(
                        self._tiff_frame_cpu(index), index
                    )
                except Exception as cpu_exc:
                    if self.device.type == "cuda" and _looks_like_cuda_oom(cpu_exc):
                        raise RuntimeError(
                            "nvTIFF was disabled and the frame was decoded on CPU, but "
                            "there is still not enough CUDA memory to transfer the "
                            "float32 processing frame. Free GPU memory, keep "
                            "--align-batch 1, or use a lower-memory processing setup."
                        ) from cpu_exc
                    raise
        else:
            image = self._cpu_array_to_device_tensor(self._tiff_frame_cpu(index), index)
...
def write_rgb_png(filename: str | Path, rgb: np.ndarray) -> None:
    """Write an 8-bit RGB PNG using only Python standard-library PNG primitives."""
    image = np.asarray(rgb)
    if image.dtype != np.uint8 or image.ndim != 3 or image.shape[2] != 3:
        raise ValueError("write_rgb_png expects a uint8 array with shape [H,W,3]")
    image = np.ascontiguousarray(image)
    height, width, _ = image.shape
    if height < 1 or width < 1:
        raise ValueError("PNG dimensions must be positive")

    # PNG filter type 0 (None) is prepended to every scan line.
...
def _display_bin_mean(image: np.ndarray, binning: int) -> np.ndarray:
    source = np.asarray(image, dtype=np.float32)
    if source.ndim != 2:
        raise ValueError("Patch displacement background must be a 2-D image")
    factor = max(1, int(binning))
    if factor == 1:
        return source.copy()
    height, width = source.shape
    factor = min(factor, height, width)
    out_h = max(1, height // factor)
    out_w = max(1, width // factor)
    trimmed = source[: out_h * factor, : out_w * factor]
    return trimmed.reshape(out_h, factor, out_w, factor).mean(axis=(1, 3))


def _grayscale_uint8_for_display(image: np.ndarray) -> np.ndarray:
    source = np.asarray(image, dtype=np.float32)
    ...
    if not np.isfinite(lo) or not np.isfinite(hi) or hi <= lo:
        ...
        hi = float(finite.max())
    if hi <= lo:
        return np.full(source.shape, 127, dtype=np.uint8)
    scaled = (source - lo) * (255.0 / (hi - lo))
    scaled = np.nan_to_num(scaled, nan=0.0, posinf=255.0, neginf=0.0)
    return np.clip(scaled, 0.0, 255.0).astype(np.uint8)


def _motion_model_device(model: MotionModel) -> torch.device:
    if isinstance(model, PolynomialMotionModel):
        return model.coeff_x.device
    if isinstance(model, SplineMotionModel):
        return model.control_x.device
    if isinstance(model, SumMotionModel):
        return _motion_model_device(model.first)
    return torch.device("cpu")


def _model_patch_displacement_at_frame(
    model: MotionModel,
    centers: np.ndarray,
    frame_index: int,
) -> Tuple[np.ndarray, np.ndarray]:
    """Evaluate smoothed local displacement relative to frame 0."""
    device = _motion_model_device(model)
    x = torch.as_tensor(centers[:, 0], dtype=torch.float32, device=device)
    y = torch.as_tensor(centers[:, 1], dtype=torch.float32, device=device)
    t0 = torch.zeros_like(x)
    tf = torch.full_like(x, float(frame_index))
    with torch.no_grad():
        sx0, sy0 = model.evaluate_points(x, y, t0)
        sxf, syf = model.evaluate_points(x, y, tf)
    dx = (sxf - sx0).detach().cpu().numpy()
    dy = (syf - sy0).detach().cpu().numpy()
    return dx, dy


def _draw_red_outline(
    rgb: np.ndarray,
    center_x: float,
    center_y: float,
    half_width: float,
    ...
    x0 = max(0, min(width - 1, round_nearest(center_x - half_width)))
    x1 = max(0, min(width - 1, round_nearest(center_x + half_width)))
    y0 = max(0, min(height - 1, round_nearest(center_y - half_height)))
    y1 = max(0, min(height - 1, round_nearest(center_y + half_height)))
    if x1 < x0 or y1 < y0:
        return
    thickness = max(1, int(line_width))
    red = np.asarray([255, 0, 0], dtype=np.uint8)
    rgb[y0 : min(y0 + thickness, y1 + 1), x0 : x1 + 1] = red
    rgb[max(y1 - thickness + 1, y0) : y1 + 1, x0 : x1 + 1] = red
    rgb[y0 : y1 + 1, x0 : min(x0 + thickness, x1 + 1)] = red
    rgb[y0 : y1 + 1, max(x1 - thickness + 1, x0) : x1 + 1] = red


def write_patch_displacement_png(
    filename: str | Path,
    ...
    model: MotionModel,
    frame_index: int,
    display_binning: int = 8,
) -> None:
    """Overlay one frame's smoothed patch boxes on the fixed final sum.

    The background is the already-generated final corrected sum. Only rectangle
    positions move; their interiors remain the original grayscale background.
    No trajectory or external plotting/image library is used.
    """
    background = np.asarray(final_image, dtype=np.float32)
    display = _display_bin_mean(background, display_binning)
    gray = _grayscale_uint8_for_display(display)
    rgb = np.repeat(gray[..., None], 3, axis=2)

    dx, dy = _model_patch_displacement_at_frame(model, centers, frame_index)
    scale_x = display.shape[1] / max(background.shape[1], 1)
    scale_y = display.shape[0] / max(background.shape[0], 1)
    half_width = max(1.0, 0.5 * float(patch_size) * scale_x)
    half_height = max(1.0, 0.5 * float(patch_size) * scale_y)

    for patch_index in range(centers.shape[0]):
        center_x = (float(centers[patch_index, 0]) + float(dx[patch_index])) * scale_x
        center_y = (float(centers[patch_index, 1]) + float(dy[patch_index])) * scale_y
        _draw_red_outline(rgb, center_x, center_y, half_width, half_height, line_width=2)

    write_rgb_png(filename, rgb)
...
def validate_arguments(args: argparse.Namespace, n_frames: int) -> None:
    ...
    if args.patch_plot_frame < 0 or args.patch_plot_frame > n_frames:
        raise ValueError("--patch-plot-frame must be 0 (last) or a valid 1-based frame number")
    if args.patch_plot_binning < 1:
        raise ValueError("--patch-plot-binning must be >= 1")
...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="auto",
        help=(
            "TIFF decode backend. auto/nvtiff attempts nvTIFF on CUDA and permanently "
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Write one PNG with smoothed patch boxes over the final corrected sum",
    )
    parser.add_argument(
        "--patch-plot-file",
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="1-based frame to display; 0 selects the final movie frame",
    )
    parser.add_argument(
        "--patch-plot-binning",
        type=int,
        default=8,
        help="Simple mean binning used only for the PNG background",
    )
...
def run_single(args: argparse.Namespace) -> int:
    ...
    with MovieSource(
        ...
        tiff_decoder=args.tiff_decoder,
    ) as movie:
        ...
        final_sum_np = final_sum.detach().cpu().numpy()
        write_mrc_image(args.output, final_sum_np, output_pixel_size)

        if args.patch_plot:
            if local_model is not None and centers is not None and patch_size is not None:
                plot_frame_index = (
                    movie.n_frames - 1
                    if args.patch_plot_frame == 0
                    else args.patch_plot_frame - 1
                )
                patch_plot_path = (
                    Path(args.patch_plot_file)
                    if args.patch_plot_file
                    else output_path.with_name(output_path.stem + "_patch_displacement.png")
                )
                write_patch_displacement_png(
                    patch_plot_path,
                    final_sum_np,
                    centers,
                    patch_size,
                    local_model,
                    plot_frame_index,
                    display_binning=args.patch_plot_binning,
                )
                log(
                    f"Patch displacement PNG: {patch_plot_path} "
                    f"(frame {plot_frame_index + 1}, display bin {args.patch_plot_binning})"
                )
            else:
                log("Patch displacement PNG was not written because no local patch model exists")
